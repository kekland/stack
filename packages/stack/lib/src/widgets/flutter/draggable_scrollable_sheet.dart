...
class DraggableScrollableController extends ChangeNotifier {
  ...
  DraggableScrollableSheetScrollController? attachedController;
  ...
  /// called.
  bool get isAttached => attachedController != null && attachedController!.hasClients;

  /// Convert a sheet's pixel height to size (fractional value of parent container height).
  double pixelsToSize(double pixels) {
    _assertAttached();
    return attachedController!.extent.pixelsToSize(pixels);
  }
  ...
  Future<void> animateTo(double size, {required Duration duration, required Curve curve}) async {
    ...
    final AnimationController animationController = AnimationController.unbounded(
      vsync: attachedController!.position.context.vsync,
      value: attachedController!.extent.currentSize,
    );
    _animationControllers.add(animationController);
    // _attachedController!.position.goIdle();
    // This disables any snapping until the next user interaction with the sheet.
    attachedController!.extent.hasDragged = false;
    attachedController!.extent.hasChanged = true;
    attachedController!.extent.startActivity(
      onCanceled: () {
    ...
    animationController.addListener(() {
      attachedController!.extent.updateSize(
        animationController.value,
        attachedController!.position.context.notificationContext!,
      );
    });
    await animationController.animateTo(
      clampDouble(size, attachedController!.extent.minSize, attachedController!.extent.maxSize),
      duration: duration,
      curve: curve,
    );
  }
  ...
  void jumpTo(double size) {
    ...
    attachedController!.extent.startActivity(onCanceled: () {});
  ...
  void _attach(DraggableScrollableSheetScrollController scrollController) {
    ...
    attachedController = scrollController;
    attachedController!.extent._currentSize.addListener(notifyListeners);
    attachedController!.onPositionDetached = _disposeAnimationControllers;
  }
  ...
  void _onExtentReplaced(DraggableSheetExtent previousExtent) {
    ...
    // the new extent.
    attachedController!.extent._currentSize.addListener(notifyListeners);
    if (previousExtent.currentSize != attachedController!.extent.currentSize) {
  ...
  void _detach({bool disposeExtent = false}) {
    if (disposeExtent) {
      attachedController?.extent.dispose();
    } else {
      attachedController?.extent._currentSize.removeListener(notifyListeners);
    }
...
class DraggableScrollableSheet extends StatefulWidget {
  ...
  @override
  State<DraggableScrollableSheet> createState() => DraggableScrollableSheetState();
}
...
class DraggableScrollableSheetState extends State<DraggableScrollableSheet> {
  late DraggableScrollableSheetScrollController scrollController;
  late DraggableSheetExtent extent;

  DraggableScrollableSheetScrollController createScrollController(DraggableSheetExtent extent) {
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<double>(
      valueListenable: extent._currentSize,
      builder: (BuildContext context, double currentSize, Widget? child) => LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          extent.availablePixels = widget.maxChildSize * constraints.biggest.height;
  ...
  void _replaceExtent(covariant DraggableScrollableSheet oldWidget) {
    final DraggableSheetExtent previousExtent = extent;
    extent = previousExtent.copyWith(
      minSize: widget.minChildSize,
      maxSize: widget.maxChildSize,
      snap: widget.snap,
      snapSizes: _impliedSnapSizes(),
    ...
    scrollController.extent = extent;
    ...
    if (widget.snap &&
      ...
      WidgetsBinding.instance.addPostFrameCallback((Duration timeStamp) {
        for (int index = 0; index < scrollController.positions.length; index++) {
          final DraggableScrollableSheetScrollPosition position =
              scrollController.positions.elementAt(index) as DraggableScrollableSheetScrollPosition;
...
class DraggableScrollableSheetScrollController extends ScrollController {
  ...
  @override
  DraggableScrollableSheetScrollPosition get position => super.position as DraggableScrollableSheetScrollPosition;
...
class DraggableScrollableSheetScrollPosition extends ScrollPositionWithSingleContext {
  DraggableScrollableSheetScrollPosition({
  ...
  void absorb(ScrollPosition other) {
    ...
    if (other is! DraggableScrollableSheetScrollPosition) {
      return;
    }
  ...
  bool shouldSnap() => extent.snap && extent.hasDragged && !_isAtSnapSize();
  ...
  void goBallisticForSheet(double velocity) {
...
class DraggableScrollableActuator extends StatefulWidget {
  ...
  static bool reset(BuildContext context) {
    final InheritedResetNotifier? notifier = context.dependOnInheritedWidgetOfExactType<InheritedResetNotifier>();
    return notifier?._sendReset() ?? false;
  }

  @override
  State<DraggableScrollableActuator> createState() => DraggableScrollableActuatorState();
}

class DraggableScrollableActuatorState extends State<DraggableScrollableActuator> {
  final ResetNotifier _notifier = ResetNotifier();

  @override
  Widget build(BuildContext context) {
    return InheritedResetNotifier(notifier: _notifier, child: widget.child);
  }
...
class InheritedResetNotifier extends InheritedNotifier<ResetNotifier> {
  ...
  static bool shouldReset(BuildContext context) {
    final InheritedWidget? widget = context.dependOnInheritedWidgetOfExactType<InheritedResetNotifier>();
    if (widget == null) {
      return false;
    }
    assert(widget is InheritedResetNotifier);
    final InheritedResetNotifier inheritedNotifier = widget as InheritedResetNotifier;
