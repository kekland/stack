...
/// A value dispatcher is a service ties together [ValueEvent] events with [ValueSource]/[ValueProxy]s or other systems
/// that need to respond to global changes to a value.
///
/// This is used to, for example, sync together multiple value sources that represent the same underlying data.
abstract class ValueDispatcher<T> extends Service {
  ValueDispatcher({required super.logger});
  ...
  void _dispatchMutationResult<TReturn>(TReturn value) {
    if (value is T) {
      ...
      logger.warning('Automatically dispatching mutation result of type $TReturn failed: cannot unwrap into $T');
    }
  }
  ...
  Future<TReturn> $mutation<TReturn>(
    Object operationId,
    Object args,
    FutureOr<TReturn> Function() mutate, {
    ...
    if (optimisticUpdate != null) {
      try {
        ...
        logger.warning('Optimistic update for $operationId failed with: $e. Ignoring.');
      }
    }

    // After optimistic updates, proceed with regular mutation.
    try {
      final result = await $method(mutate);
...
extension DispatcherDiExtensions on GetIt {
